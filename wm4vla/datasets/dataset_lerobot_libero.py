...
Training objective:
  (cam1_t, cam2_t, task, [a_t...a_{t+d-1}], delay_norm)  →  (cam1_{t+d}, cam2_{t+d})

Each dataset item returns a paired mini-batch with two single-view samples:
  Sample 0 (cam1): [cam1_t, cam1_{t+d} ×4]
  Sample 1 (cam2): [cam2_t, cam2_{t+d} ×4]

5-frame video layout per view (state_t=2, num_conditional_frames=1):
  Frame 0      : view_t                         (latent 0 – conditioning)
  Frames 1–4   : view_{t+d} ×4                 (latent 1 – predicted)

action       : [2, 8, 8] float32  — masked [a_t ... a_{t+d-1}]
delay_scalar : [2, 1] float32     — normalized global delay
...
Max window needed: max_delay + 1 = 9.

cam1 = observation.images.image       (agentview, 3rd-person)
cam2 = observation.images.wrist_image (wrist / 1st-person)

Default data root:
  /home/kyji/storage_net/tmp/lbai/cosmos-predict2.5/lerobot/lerobot--libero_spatial_image@v2.0
...
class LeRobotLiberoDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            max_delay: Fixed action chunk length expected by the model.
            sampled_delay_max: Curriculum sampling upper bound for d.
                Defaults to max_delay and must satisfy sampled_delay_max <= max_delay.
            fixed_delay: If set, always train on this exact delay instead of
                sampling from [TRAIN_DELAY_MIN, sampled_delay_max]. Useful for
                single-delay ablations.
            delay_normalization_max: Denominator used for global delay
                normalization. Defaults to max_delay.
        ...
        # Valid start: start_t + effective_delay_max ≤ T - 1.
        ...
        for file_path in selected_files:
            ...
            if self.task_indices is not None:
                ...
                if ep_task_index not in self.task_indices:
                    continue
            T = len(df_meta)
            max_valid_start = T - self._window_delay_max
        ...
        print(
            ...
            f"{fixed_delay_str}{task_filter_str}, data_root={data_root})"
        )
    ...
    def _load_sample(self, index: int) -> dict:
        ...
        action = torch.from_numpy(action_seq).unsqueeze(0).repeat(2, 1, 1)  # [2, max_delay, 8]
        delay_scalar = torch.from_numpy(delay_scalar).unsqueeze(0).repeat(2, 1)  # [2, 1]
        ...
        return {
            ...
            "action": action,                                  # [2, max_delay, 8] float32
            "delay_scalar": delay_scalar,                      # [2, 1] float32
            "__key__": key,
